...
class CinescopMoviePage(BasePage):
    def __init__(self, page: Page):
        ...
        self.review_input = "textarea[name='text']"
        self.rating_select = "button[role='combobox']"
        self.submit_button = "button[type='submit']"
        self.review_text_locator = "ul li"

    def open(self, movie_id: int):
        self.open_url(f"{self.home_url}movies/{movie_id}")

    @allure.step("Написать отзыв: {text}")
    def enter_review(self, text: str):
    ...
    def has_review_form(self) -> bool:
        try:
            self.page.locator(self.review_input).wait_for(state="visible", timeout=5000)
            return True
        except Exception:
            return False

    @allure.step("Проверка что отзыв появился на странице")
